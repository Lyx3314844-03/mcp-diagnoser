...
// 测试类别
const categories = {
  core: { name: '核心诊断功能', tests: [] },
  search: { name: '搜索功能', tests: [] },
  crawl: { name: '爬取功能', tests: [] },
  proxy: { name: '代理检测', tests: [] },
 ...
 * 运行命令测试
 */
async function runCommandTest(name, command, args, expectedOutput, timeout = 30000) {
  results.total++;
  ...
  try {
    ...
    if (expectedOutput.test(output)) {
      ...
      console.log(chalk.gray(`     预期：${expectedOutput}`));
      console.log(chalk.gray(`     实际：${output.substring(0, 200)}...`));
      results.failed++;
...
function printReport() {
  ...
  if (results.skipped > 0) {
    console.log(chalk.yellow(`  跳过：${results.skipped}`));
  }
  ...
  if (results.failed === 0) {
    ...
    console.log(chalk.yellow(`  ⚠️  有 ${results.failed} 个测试失败，请检查输出。\n`));
  }
...
async function runAllTests() {
  console.log(chalk.cyan('════════════════════════════════════════════════════════════'));
  console.log(chalk.cyan('  MCP Diagnoser 完全功能测试'));
  console.log(chalk.cyan('════════════════════════════════════════════════════════════\n'));
  ...
  await runCommandTest(
    '优化版缓存预热',
    'node',
    ['dist/index-optimized.js', 'warmup'],
    /Cache warmed up/,
    60000
  );
  ...
  await runModuleTest(
    ...
    async () => {
      ...
      return {
        ...
        details: `元数据字段：${Object.keys(result.pages[0]?.metadata || {}).length}`,
      };
    },
    30000
  );
  ...
  await runModuleTest(
    'HTML 解析器 - Google 格式',
    async () => {
      const { createParser } = await import('./dist/utils/html-parser-v2.js');
      const html = `<div class="g"><div class="yuRUbf"><a href="https://example.com"><h3>测试标题</h3></a></div></div>`;
  ...
  console.log(chalk.gray('  测试结果已保存到：test-results.json\n'));
